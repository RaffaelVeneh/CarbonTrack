...
export default function WeeklyMissionsTab({ userId, API_URL, onActivitySelect, onClaimSuccess, refreshKey }) {
    ...
    if (isLoading && weeklyMissions.length === 0) {
        return <div className="text-center py-20 text-gray-500">Loading misi mingguan...</div>;
    }
                    ...
                    return (
                        <div
                            ...
                            className={`relative bg-white rounded-xl p-5 shadow-md border-2 transition-all
                                ${isClaimed ? 'border-gray-300 opacity-60' : 'border-blue-200 hover:shadow-lg'}`}
                        >
                            {/* Difficulty Badge */}
                            <div className={`absolute top-3 right-3 px-2 py-1 rounded-full text-xs font-bold ${difficultyStyle.color}`}>
                                    ...
                                    {!isClaimed && (
                                        <div className="mb-3">
                                            <div className="flex justify-between text-xs mb-1.5">
                                                ...
                                                <span className="font-bold text-blue-600">{mission.progress_text}</span>
                                            </div>
